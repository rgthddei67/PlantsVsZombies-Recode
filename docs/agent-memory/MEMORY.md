...
> 当前数值/实现直接查源码或权威配置；本索引用于寻找设计原因、例外和排查入口。下方旧数值与测试摘要保留为历史快照，不要求随调参更新。记录规则见 [AGENTS.md](../../AGENTS.md#文档与记忆)。

- [矿场9-5/9-6：棱光花、盗晶与紫雾](../superpowers/specs/2026-09-11-gloomcrystal-mine-9-5-9-6-design.md) — 已批准、尚未实现；接续本组关卡设计时读取。
...
- [第八大关极光祭司、极夜钟匠与终盘植物](project_pvz_area8_rift_clockwork_finale.md) — 2026-08-30 已实现 8-7/8-8：祭司提交后5秒循环且累计最多释放3次（中断不计，次数存档并随钟匠记录回溯），钟匠提交后10秒循环，2026-09-11祭司场外行走速度×2且仍须入场前摇，钟匠可在场外前摇；2026-09-05解除能力提交/读档的极夜地图限制并适配矿道连续回位；时间锚6秒后恢复核心状态，并精确恢复祭司/钟匠局部阶段及适应头盔的 NONE/植物谱系/ASH，存活目标的断头/断臂逻辑与异形骨架轨道会在原对象静默重建，其余品种不承诺内部技能倒带，外部 Board 事务不回滚且来源不自刷；界碑花改道离散入场，曙光莲结算三红模块；schema 12、v11提交态迁移、独立逆转钟面和复合粒子已闭环
...
- [选卡界面翻页与模仿者独立入口](project_pvz_choose_card_pagination.md) — 2026-09-11 普通卡仍48张分页；模仿目标独立9×5分页，当前54目标分45/9，隐藏卡禁输入、底部箭头页码、取消重开归首页；可见clang-release分页与原UI回归通过
