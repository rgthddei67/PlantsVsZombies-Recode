...
`audit_commander_forecast.py <output>` 对正常付费的一批队伍运行其战斗预测时域，禁止后续增援；加 `--policy <候选> --arenas <场景...> --opponent <陪练>` 可检查冻结候选。新模型交战窗口为 90 秒、旧模型为 60 秒，产冰始终按前 60 秒的事件记录结算。它隔离了追加购买的干扰，但植物陪练会真实补阵，因此差距也包含推演尚未表达的后续补阵。`searchFeatures`、`searchBaselineFeatures` 和 `searchPreferenceScore` 分别解释计划、无增援基线和兵种经验加分；逐场 `decisions` 记录搜索编号、付款和预测，包含观望。这些诊断不进玩家存档。
