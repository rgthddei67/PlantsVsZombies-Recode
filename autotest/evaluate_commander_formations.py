"""Frozen-policy transfer test against unseen elite mushroom or ash-only defenses."""
import argparse
import hashlib
import json
...
def evaluate(args):
    ...
    identity = {'policySha256': hashlib.sha256(args.policy.read_bytes()).hexdigest(),
                ...
                'seed': args.seed, 'seconds': args.seconds, 'opponent': args.opponent}
    ...
    if args.opponent == 'ash':
        cases = [(arena,'ash',args.seed + n*7919 + offset,args.seconds) for n in range(3)
                 for arena,offset in [('opening',31),('opening_10_2',67)]]
    ...
    print(args.opponent + ' wins:', [sum(r['outcome'] == 'commander_win' for r in rows) for rows in scores])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--opponent', choices=('elite','ash'), default='elite')
