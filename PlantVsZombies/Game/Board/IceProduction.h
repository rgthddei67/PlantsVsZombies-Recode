...
inline constexpr float Interval = 3.6f; // 每批生产间隔，游戏秒，成长不缩短间隔
inline constexpr float InitialYield = 4.0f; // 初次每批产量，冰块
inline constexpr float YieldGrowth = 1.5f; // 每次成功生产后下批产量倍率
inline constexpr float MaximumYield = 18.0f; // 单只每批产量上限，冰块；限制受保护工人的长期滚雪球
