...
bool TestDriver::ExecuteCurrent() {
	...
	if (op == "commander_experiment") {
		if (!ColdStoragePolicy::SetExperiment(cmd.value("weights", nlohmann::json()),
			...
			cmd.contains("stateModel") ? &cmd.at("stateModel") : nullptr)) {
...
bool TestDriver::BuildStateJson(const std::string& opName, nlohmann::json& out)
{
	...
	if (board->IsColdStorage()) {
		...
		for (int row = 0; row < board->mRows; ++row)
			ice["splashRiskByRowOn100"].push_back(static_cast<int>(std::lround(board->mColdStorage.splashRiskByRow[row] * 100.0f)));
		for (int row = 0; row < board->mRows; ++row)
		...
		for (int row = 0; row < board->mRows; ++row) ice["economyRows"].push_back({
			...
			{"entryDelayMs", static_cast<int>(std::lround(board->mColdStorage.economyEntryDelayByRow[row] * 1000.0f))},
			{"coverMs", static_cast<int>(std::lround(board->mColdStorage.economyCoverByRow[row] * 1000.0f))}});
